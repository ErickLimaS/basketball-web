...
export default function FebruarySeason(item) {
    ...
    useEffect(() => {
        const load = async () => {
            const { data } = await API.GetSelectSeasonFebruary(season, id)
            setMonthSeason(data)
        }
        load()
    }, [id, season])
    return (
        <C.Container>
            <div className='month-score'>
                <h3 className='month-heading'>February/{Number.parseInt(season) + 1}</h3>
                <div className='month-score'>
